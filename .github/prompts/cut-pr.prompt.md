...
- **Ticket Identifier:** {{TICKET_ID}} (GitHub issue number or Jira ticket key, optional)
- **Current branch:** current workspace

## Phase 0: Confirm Inputs
0.1 Echo the target branch: {{BRANCH_NAME}} and Ticket Identifier: {{TICKET_ID}} (if provided)
...
### Title
Generate {{PR_TITLE}} based on the changes made and the Ticket Identifier: {{TICKET_ID}} (if provided).
This must be a semantic title (remember to place the GitHub issue number right after the : if there is a ticket identifier).
